...
for (const noGpu of [false, true]) {
  ...
  const before = await p.evaluate(()=>window.__tenkyu);
  // Click the Manhattan fly-to button (generated from the manifest).
  await p.getByTitle('fly to New York (Manhattan)').click();
  await p.waitForFunction(()=> (window.__tenkyu?.buildings??0) > 0, {timeout:60000})
    .catch(()=>{});
  await p.waitForTimeout(6000);
  const after = await p.evaluate(()=>window.__tenkyu);
  ...
  await p.locator('#tenkyu-globe').screenshot({path:`test/browser/city-${noGpu?'webgl2':'webgpu'}.png`});
  await p.close();
}
await b.close();
